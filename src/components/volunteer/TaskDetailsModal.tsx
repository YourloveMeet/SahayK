...
export function TaskDetailsModal({ task, distance, isOpen, onClose, onAccept }: TaskDetailsModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/60 backdrop-blur-md h-screen w-screen overflow-y-auto" onClick={onClose}>
      ...
      >
        {/* Header Header */}
        <div className={`p-6 border-b border-gray-100 dark:border-zinc-800 flex items-start justify-between ${task.is_urgent ? 'bg-gray-100 dark:bg-zinc-800' : 'bg-gray-50 dark:bg-black/20'}`}>
          <div className="pr-4">
            <div className="flex items-center gap-2 mb-3">
              <span className="inline-flex items-center gap-1.5 px-3 py-1 bg-white dark:bg-zinc-800 text-gray-900 dark:text-gray-100 text-xs font-bold rounded-lg border border-gray-200 dark:border-zinc-700 shadow-sm">
                <CategoryIcon className="w-4 h-4" /> {categoryLabel}
              </span>
              {task.is_urgent && (
                <span className="px-3 py-1 bg-gray-900 dark:bg-white text-white dark:text-gray-900 text-xs font-black uppercase tracking-widest rounded-lg shadow-md">
                  Urgent
                </span>
              )}
            </div>
        ...
        <div className="p-8 space-y-8 flex-1">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="p-4 rounded-2xl bg-gray-50 dark:bg-zinc-800/50 border border-gray-100 dark:border-zinc-800">
              ...
              <p className="font-bold text-gray-900 dark:text-white flex items-center gap-2">
                <User className="w-5 h-5 text-gray-500" /> {task.profiles?.full_name}
              </p>
            </div>
            <div className="p-4 rounded-2xl bg-gray-50 dark:bg-zinc-800/50 border border-gray-100 dark:border-zinc-800">
              <p className="text-sm font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider mb-1">Distance</p>
              <p className="font-bold text-gray-900 dark:text-white flex items-center gap-2">
                <Ruler className="w-5 h-5 text-gray-500" /> {distance !== undefined ? (distance < 1 ? '< 1 km away' : `${distance.toFixed(1)} km away`) : 'Distance unknown'}
              </p>
            </div>
          </div>
          ...
          <div>
            ...
            <div className="p-4 rounded-2xl bg-gray-50 dark:bg-zinc-800/50 border border-gray-100 dark:border-zinc-800 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <div className="flex items-start gap-3">
                <MapPin className="w-6 h-6 text-gray-900 dark:text-white shrink-0" />
                <p className="font-bold text-gray-900 dark:text-white leading-snug pt-0.5">{task.area_name || 'Location hidden until accepted'}</p>
              </div>
              {task.latitude && task.longitude && (
                <a 
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="shrink-0 flex items-center gap-2 px-4 py-2 bg-white dark:bg-zinc-800 hover:bg-gray-50 dark:hover:bg-zinc-700 text-gray-900 dark:text-white font-bold text-sm rounded-xl border border-gray-200 dark:border-zinc-700 shadow-sm transition-colors"
                >
        ...
        <div className="p-6 border-t border-gray-100 dark:border-zinc-800 bg-gray-50 dark:bg-black/20 flex items-center justify-between">
          <p className="text-sm text-gray-500 dark:text-gray-400 font-medium pr-4">
          ...
          <button
            ...
            className="px-8 py-4 bg-gray-900 dark:bg-white hover:bg-black dark:hover:bg-gray-100 text-white dark:text-gray-900 font-black text-lg rounded-xl shadow-xl transition-all transform hover:-translate-y-1 active:translate-y-0 flex items-center gap-3 shrink-0"
          >
            <Handshake className="w-6 h-6" /> Accept Request
          </button>
        </div>
      </div>
    </div>
