...
export default function DonorLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex min-h-screen bg-zinc-50 dark:bg-[#0A0A0A]">
      ...
      {isMobile && (
        <div className="fixed bottom-6 left-4 right-4 z-50 bg-zinc-950 border border-zinc-800 h-16 rounded-2xl px-6 flex items-center justify-center gap-12 shadow-2xl shadow-black/40">
          {links.map((link) => {
            const isActive = pathname.startsWith(link.href)
            const Icon = link.icon
            return (
