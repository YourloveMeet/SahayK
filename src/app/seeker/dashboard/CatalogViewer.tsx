    ...
    <>
      <div className="space-y-12">
        {categories?.map((category, categoryIndex) => {
          ...
          // Define themes with a subtle background gradient and a RICH gradient icon
          const colorThemes = [
            { bg: "from-white to-orange-50/80 dark:from-zinc-900 dark:to-orange-900/20", border: "border-orange-100 dark:border-orange-900/50", hoverBorder: "hover:border-orange-300 dark:hover:border-orange-700/80", textHover: "group-hover:text-orange-700 dark:group-hover:text-orange-400", iconGradient: "bg-gradient-to-br from-orange-400 to-red-500 shadow-orange-500/30" },
            { bg: "from-white to-emerald-50/80 dark:from-zinc-900 dark:to-emerald-900/20", border: "border-emerald-100 dark:border-emerald-900/50", hoverBorder: "hover:border-emerald-300 dark:hover:border-emerald-700/80", textHover: "group-hover:text-emerald-700 dark:group-hover:text-emerald-400", iconGradient: "bg-gradient-to-br from-emerald-400 to-teal-500 shadow-emerald-500/30" },
            { bg: "from-white to-rose-50/80 dark:from-zinc-900 dark:to-rose-900/20", border: "border-rose-100 dark:border-rose-900/50", hoverBorder: "hover:border-rose-300 dark:hover:border-rose-700/80", textHover: "group-hover:text-rose-700 dark:group-hover:text-rose-400", iconGradient: "bg-gradient-to-br from-rose-400 to-pink-500 shadow-rose-500/30" },
            { bg: "from-white to-amber-50/80 dark:from-zinc-900 dark:to-amber-900/20", border: "border-amber-100 dark:border-amber-900/50", hoverBorder: "hover:border-amber-300 dark:hover:border-amber-700/80", textHover: "group-hover:text-amber-700 dark:group-hover:text-amber-400", iconGradient: "bg-gradient-to-br from-amber-400 to-yellow-500 shadow-amber-500/30" },
            { bg: "from-white to-purple-50/80 dark:from-zinc-900 dark:to-purple-900/20", border: "border-purple-100 dark:border-purple-900/50", hoverBorder: "hover:border-purple-300 dark:hover:border-purple-700/80", textHover: "group-hover:text-purple-700 dark:group-hover:text-purple-400", iconGradient: "bg-gradient-to-br from-purple-500 to-fuchsia-600 shadow-purple-500/30" },
          ];
          const theme = colorThemes[categoryIndex % colorThemes.length];
            ...
            <div key={category.id} className="space-y-6">
              ...
              <div className="flex overflow-x-auto pb-8 pt-2 px-2 -mx-2 snap-x snap-mandatory hide-scrollbar gap-5 after:content-[''] after:shrink-0 after:w-4">
                {sortedServices.map((service) => (
                  ...
                  >
                    <div className={`absolute -inset-1 bg-gradient-to-br ${theme.bg} rounded-3xl transition-all duration-300 opacity-0 group-hover:opacity-100 blur-sm`}></div>
                    
                    <div className={`h-full flex flex-col justify-between p-6 bg-gradient-to-br ${theme.bg} border ${theme.border} rounded-3xl shadow-sm hover:shadow-xl ${theme.hoverBorder} transition-all duration-300 hover:-translate-y-1 relative z-10 overflow-hidden`}>
                      ...
                      <div className="flex items-center justify-between mt-2">
                        <div className={`h-8 w-8 rounded-full ${theme.iconGradient} shadow-md flex items-center justify-center transition-transform duration-300 group-hover:scale-110`}>
                          <svg className="w-4 h-4 text-white transform group-hover:translate-x-0.5 transition-transform" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={3}>
            ...
            <div className="flex-1 overflow-y-auto p-6 md:p-8">
              <div className="grid md:grid-cols-2 gap-8 md:gap-12 pb-4">
                ...
                <div className="space-y-8">
                  <div>
                    <div className="inline-block px-4 py-1.5 bg-emerald-100 dark:bg-emerald-500/20 text-emerald-800 dark:text-emerald-300 font-bold rounded-full text-sm uppercase tracking-wider mb-4">
                  ...
                  <div className="space-y-4">
                    {selectedService.steps?.map((step: string, idx: number) => (
                      <div key={idx} className="flex items-start gap-4">
                        <div className="shrink-0 w-8 h-8 rounded-full bg-emerald-50 dark:bg-emerald-900/30 flex items-center justify-center font-bold text-emerald-600 dark:text-emerald-400 border border-emerald-200 dark:border-emerald-800">
                          {idx + 1}
                        </div>
                ...
                <div className="flex flex-col h-full">
                  <div className="mb-6">
                    <div className="inline-block px-4 py-1.5 bg-indigo-100 dark:bg-indigo-500/20 text-indigo-800 dark:text-indigo-300 font-bold rounded-full text-sm uppercase tracking-wider">
                      Option 2: Need Help?
                    </div>
                  </div>

                  <div className="flex-1 p-6 md:p-8 bg-indigo-50 dark:bg-indigo-950/30 border border-indigo-100 dark:border-indigo-900/50 rounded-3xl shadow-inner relative overflow-hidden flex flex-col">
                    
                    {/* Decorative blur */}
                    <div className="absolute top-0 right-0 w-32 h-32 bg-indigo-400/10 rounded-full blur-3xl pointer-events-none"></div>

                    <p className="text-indigo-900 dark:text-indigo-200 font-medium text-lg leading-relaxed relative z-10 mb-8">
                      If you cannot complete this yourself, a SahayaK volunteer can assist you in person.
                    </p>

                    <div className="space-y-6 relative z-10 mb-8">
                      <div className="flex items-start gap-4">
                        <div className="w-10 h-10 rounded-full bg-white dark:bg-black/20 flex items-center justify-center shrink-0 shadow-sm border border-indigo-100 dark:border-indigo-800">
                    ...
                    <div className="mt-auto relative z-10 pt-2">
                      <Link 
                        ...
                        className="flex items-center justify-center gap-2 w-full px-6 py-5 bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-500 hover:to-purple-500 text-white font-black text-lg rounded-xl transition-all shadow-lg shadow-indigo-500/25 hover:shadow-indigo-500/40 hover:-translate-y-1"
                      >
                        Request a Volunteer
